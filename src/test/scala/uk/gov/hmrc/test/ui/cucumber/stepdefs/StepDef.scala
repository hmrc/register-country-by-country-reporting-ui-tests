...
class StepDef extends BaseStepDef {
  ...
  Given("""^(.*) logs in to register for CBC$""") { name: String =>
    name match {
  ...
  Then("""^I navigate to (.*) page$""") { page: String =>
    page match {
      case "start"      =>
        Nav.navigateTo("http://localhost:10026/register-country-by-country-reporting-frontend/register/have-utr")
      case "individual" =>
        Nav.navigateTo(
          "http://localhost:10026/register-country-by-country-reporting-frontend/register/problem/individual-sign-in-problem"
        )
    }
  }
